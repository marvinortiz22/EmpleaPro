...
public class vacacionesDAOImpl implements vacacionesDAO{
    ...
    public List<Vacacion> getVacacionesAñoActual() {
        Session session=sessionFactory.getCurrentSession();
        Query<Object[]> query=session.createNativeQuery("SELECT idvacacion,e.idempleado,v.fechainicio FROM vacacion v right JOIN empleado e ON e.IDEMPLEADO=v.IDEMPLEADO AND YEAR(fechainicio)=YEAR(CURDATE()) where e.estado=1",Object[].class);
